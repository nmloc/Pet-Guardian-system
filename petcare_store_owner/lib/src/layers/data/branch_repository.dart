...
class BranchRepository {
  ...
  Stream<Branch> watchBranch(String branchId) => queryBranches()
      .doc(branchId)
      .snapshots()
  ...
  Future<(List<Branch>, DocumentSnapshot?)> lazyFetch(
      int limit, DocumentSnapshot? lastOrder) async {
    return await (lastOrder == null
            ? queryBranches().limit(limit)
            : queryBranches().limit(limit).startAfterDocument(lastOrder))
        .get()
        .then(
